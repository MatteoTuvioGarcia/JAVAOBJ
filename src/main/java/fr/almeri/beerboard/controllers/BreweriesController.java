...
//    }
    ...
    public String ajouterBrasserie(@Validated @ModelAttribute Brasserie brasserie, Model model, HttpSession session) {
        if (session.getAttribute("auth") != null) {// Création d'une brasserie + enregistrement dans la base de données.
    ...
    public String SupprimerBrasserie(@Validated @ModelAttribute Brasserie brasserie, Model model, HttpSession session, RedirectAttributes message) {
        if (session.getAttribute("auth") != null) {// suppression d'une brasserie

            ArrayList<Biere> bieres = biereRepository.getBierefromBrasserie(brasserie.getCodeBrasserie());
            if (bieres.isEmpty()) {
                message.addFlashAttribute("messagehtml", " La brasserie " + brasserie.getNomBrasserie() + " a bien été supprimée.");
                brasserieRepository.delete(brasserie);
            } else {
                message.addFlashAttribute("messageerror", " La brasserie " + brasserie.getNomBrasserie() + " n'a pas pu être supprimée, elle possède des bières associées.");
            }
            return "redirect:/breweries";
        } else {
            // Page de connexion
